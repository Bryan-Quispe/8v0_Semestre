...
\textbf{Alcance} & MVP en desarrollo: Flujos principales y alternos detallados \\
\end{tabular}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Backend valida reglas de negocio: email \u00fanico, contrase\~na cumple pol\'itica (>=8 caracteres, opcional: may\'usculas/n\'umeros).
...
\begin{itemize}[leftmargin=*]
    \item RB-01: Email debe ser \u00fanico en la tabla users.
...
\textbf{Flujo normal}:
\begin{enumerate}[leftmargin=*]
    \item Usuario completa formulario: nombre, edad, tipo, (foto opcional).
    \item Frontend valida campos y adjunta token en Authorization header.
    \item Backend valida token, extrae userId y valida campos obligatorios.
    \item Si hay foto, backend la procesa (comprimir/validar tipo y tama\~no) y guarda en almacenamiento.
    \item Backend crea registro en tabla mascotas con referencia al usuario y ruta de foto/modelo.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Backend valida token, consulta DB por userId, aplica paginaci\'on si procede.
    \item Backend retorna lista en JSON con campos: id, nombre, edad, tipo, urlFoto, fechaCreacion.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Backend retorna detalles y la URL del modelo 3D (GLB/GLTF) asociado.
    \item Frontend carga el GLB con Three.js y renderiza en canvas interactivo.
\end{enumerate}
...
\newpage
\section{CU-07: Subir modelo 3D (Admin)}
\begin{tabular}{|p{3.5cm}|p{10cm}|}
\hline
\textbf{Identificador} & CU-07 \\
\hline
\textbf{Nombre} & Subir modelo 3D al cat\'alogo \\
\hline
\textbf{Actor principal} & Administrador (admin) \\
\hline
\textbf{Actores secundarios} & Sistema interno \\
\hline
\textbf{Precondiciones} & Admin autenticado con permisos de publicaci\'on. \\
\hline
...
\textbf{Flujo normal}:
\begin{enumerate}[leftmargin=*]
    \item Admin sube archivo GLB/GLTF con metadatos (nombre, tipo, versi\'on).
    \item Backend valida formato y tama\~no, guarda archivo en almacenamiento y registra metadatos en DB.
...
\newpage
\section{CU-08: Editar modelo 3D (Usuario)}
\begin{tabular}{|p{3.5cm}|p{10cm}|}
\hline
\textbf{Identificador} & CU-08 \\
\hline
\textbf{Nombre} & Editar / personalizar modelo 3D \\
\hline
\textbf{Actor principal} & Usuario autenticado \\
\hline
\textbf{Actores secundarios} & Administrador \\
\hline
\textbf{Precondiciones} & Usuario autenticado; modelo disponible y con campos editables permitidos. \\
\hline
\textbf{Requisitos relacionados} & RF-07 \\
\hline
\end{tabular}
...
\textbf{Flujo normal}:
\begin{enumerate}[leftmargin=*]
    \item El usuario selecciona el modelo 3D que desea personalizar.
    \item El sistema valida permisos y determina qu\'e campos pueden editarse.
    \item El usuario modifica los campos permitidos, como nombre visible, color o metadatos asociados.
    \item El sistema guarda los cambios y actualiza la versi\'on visible del modelo.
...
\subsection*{Postcondiciones}
\begin{itemize}[leftmargin=*]
    \item Metadatos actualizados o solicitud de revisi\'on registrada.
\end{itemize}

\subsection*{Criterios de aceptaci\'on}
Metadatos actualizados correctamente o solicitud registrada para revisi\'on por admin.
